...
export default function ListaAdheridos({ adheridos, cambios, onAgregarCambio, onQuitarCambio, solicitudActiva }) {
  const [editando, setEditando] = useState(null); // dni del adherido en edición
  ...
  const handleEditar = (adherido) => {
    setAgregando(false);
    setEditando(adherido.CliDocNro);
  };

  const handleEliminar = (adherido) => {
    const existente = cambioParaDni(adherido.CliDocNro);
    if (existente?.tipo === "eliminar") {
      ...
      onAgregarCambio({
        tipo: "eliminar",
        adheridoDni: adherido.CliDocNro,
        datos: adherido,
      });
      setEditando(null);
    }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100">
      ...
      {adheridos.map((a) => {
        const cambio = cambioParaDni(a.CliDocNro);
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  DNI: {a.CliDocNro} &middot; Nac: {formatFecha(a.CliFecNac)}
                </p>
...
function formatFecha(str) {
  if (!str) return "—";
  // Soporta ISO string o formato YYYY-MM-DD
